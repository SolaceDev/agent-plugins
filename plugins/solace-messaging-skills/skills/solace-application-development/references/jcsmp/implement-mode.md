...
- The one-line comments this skill mandates elsewhere (the Quickstart fail-fast posture below, the Non-Secure caveat in `solace-suggested-mode.md`, the `setCorrelationKey` distinction in `implement-request-reply.md`) always apply on their paths; this contract adds to them, it does not replace them.
