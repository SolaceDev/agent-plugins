...
3. Best practice: check `session.isCapable(CapabilityType.ENDPOINT_MANAGEMENT)` before provisioning; exit cleanly if the broker disallows client-side endpoint management.
...
9. KEEP the long-running loop and the SIGINT graceful-shutdown hook from the consumer sample. The Subscriber is the SIGINT target: on SIGINT it stops the flow, finishes outstanding ACKs, and calls `session.closeSession()`.
...
5. EXIT on its own: after observing the ACK (the `VERIFY: PUBLISH_ACKED` marker), drain outstanding ACKs (the sample's `Thread.sleep(1500)` idiom), call `session.closeSession()`, and return from `main`. REMOVE the sample's ENTER/SIGINT loop: the Publisher must terminate by itself, not wait for a key press or a signal. The SIGINT shutdown hook belongs to the Subscriber, not the Publisher.
